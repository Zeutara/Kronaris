        ...
        <div className="reveal relative">
          ...
          <div className="relative rounded-2xl border border-rule-2 overflow-hidden bg-gradient-to-b from-bg-3 to-bg-2">
            ...
            <div className="grid grid-cols-[1.4fr_1fr_1fr] border-b border-rule">
              ...
              <div className="py-4 px-5 text-center border-l border-rule bg-gold/[0.05]">
                <span className="text-[13px] uppercase tracking-wider text-gold font-medium">Kronaris</span>
              </div>
            </div>
            ...
            {rows.map((r, i) => (
              <div key={i} className={`grid grid-cols-[1.4fr_1fr_1fr] ${i < rows.length - 1 ? 'border-b border-rule/40' : ''} group hover:bg-white/[0.015] transition-colors`}>
                ...
                <div className="py-5 px-5 flex items-center justify-center border-l border-rule/30 text-center bg-gold/[0.025]">
                  <span className="text-[14px] text-t1 font-medium">{r.kronaris}</span>
                </div>
              </div>
            ))}
          </div>
